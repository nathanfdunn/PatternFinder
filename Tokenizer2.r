...
spikeDetHybrid = function( chunk, time = 1:length(chunk), bound = 3, signif=defSignif/5){
	zAlpha = abs(qnorm(signif))
	dips = sum(scale(chunk) < - zAlpha)
	med = median(chunk)
	spikes = sum(chunk > med*bound)
	cat("Spikes:", spikes)
	cat("\nDips:", spikes)
	cat("\n\n")
	return(sqign(spikes - dips))
}


spikeDetEmp = function( chunk, time=1:length(chunk), bound=3){
	print(chunk)
	med = median(chunk)
